...
def testBuilderConstructor3():
  ...
  # Settings for the neural network
  assert info['layer_units'] == [10,20,1], "Layer units didn't match user setting."
  assert info['activationfn'] == None, "Activation functions didn't match user \
  setting."
  assert info['bias'] == False, "Bias didn't match user setting."
  # Settings for the trainning
  assert info['errfn'] == "logitError", "Error function didn't match user setting."
...
def testSetter1():
  ...
  assert b.get("X") == np.array([[1,2],[3,4],[5,6]]), "Input testing data didn't match expected \
  setting."
...
def testSetterErrors():
  ...
  try:
    ...
    assert False, "Should raise an AssertionError for invalid activation function name."
  ...
  try:
    b.set(errfn="dummy")
    assert False, "Should raise an AssertionError for invalid error function name."
  except AssertionError:
    pass
  try:
    b.set(bias=10)
